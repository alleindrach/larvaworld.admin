...
class Box extends PureComponent {
  ...
  render() {
    const { box, loading, dispatch } = this.props;
    ...
    return (
      <PageHeaderWrapper title="分享爱-礼盒" content={content} extraContent={extraContent}>
          ...
          <List
            ...
            pagination={{
              onChange: (page, pageSize) => {
                while (page * pageSize > box.list.length) {
                  const fromIndex = box.list.length;
              ...
              pageSize: box.pagination.pageSize,
              // current:slides.pagination.current,
              total: box.pagination.total,
            }}
            rowKey="id"
            loading={loading}
            // grid={{ gutter: 24, lg: 3, md: 2, sm: 1, xs: 1 }}
            dataSource={['', ...box.list]}
            renderItem={item => <GiftBoxListItem data={item} />}
          />
        </div>
      </PageHeaderWrapper>
    );
